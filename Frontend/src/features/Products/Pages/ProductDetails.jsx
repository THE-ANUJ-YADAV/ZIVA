  ...
  variants.forEach((variant) => {
    if (variant.attributes) {
      Object.entries(variant.attributes).forEach(([key, value]) => {
        ...
        if (!attrs[normKey])
          attrs[normKey] = { originalKey: key.trim(), values: new Map() };
...
const ProductDetails = () => {
          ...
          keys.forEach((k) => {
            if (!initialAttrs[k]) {
              ...
              validVariants.forEach((v) => {
                const vKey = Object.keys(v.attributes || {}).find(
                  (vk) => vk.toLowerCase() === k.toLowerCase(),
                );
                if (vKey && v.attributes[vKey]) {
                  availableVals.add(
                    String(v.attributes[vKey]).trim().toLowerCase(),
                  );
                }
              });
              ...
              if (availableVals.size === 1) {
                ...
                const origVal = availableAttrs[k].find(
                  (val) => String(val).trim().toLowerCase() === normVal,
                );
                if (origVal) {
  ...
  const availableAttributes = useMemo(
    () => getAvailableAttributes(product?.variants),
    [product],
  );
  ...
  const selectedVariant = useMemo(() => {
    ...
    return product.variants.find((variant) => {
      ...
      return attributeKeys.every((key) => {
        ...
        const varAttrKey = Object.keys(variant.attributes).find(
          (k) => k.toLowerCase() === key.toLowerCase(),
        );
        return (
          varAttrKey &&
          String(variant.attributes[varAttrKey]).trim().toLowerCase() ===
            String(selectedVal).trim().toLowerCase()
        );
      });
    });
  ...
  const handleAttributeSelect = (attrName, attrValue) => {
    setSelectedAttributes((prev) => {
      ...
      let validVariants = product.variants.filter((v) => {
        const vKey = Object.keys(v.attributes || {}).find(
          (vk) => vk.toLowerCase() === attrName.toLowerCase(),
        );
        return (
          vKey &&
          String(v.attributes[vKey]).trim().toLowerCase() ===
            String(attrValue).trim().toLowerCase()
        );
      });
        ...
        if (k !== attrName && prev[k]) {
          const candidateVariants = validVariants.filter((v) => {
            ...
            return (
              vKey &&
              String(v.attributes[vKey]).trim().toLowerCase() ===
                String(prev[k]).trim().toLowerCase()
            );
          });
      ...
      while (changed) {
        ...
        attributeKeys.forEach((k) => {
          if (!resolved[k]) {
            ...
            if (availableVals.size === 1) {
              const normVal = Array.from(availableVals)[0];
              const origVal = availableAttributes[k].find(
                (val) => String(val).trim().toLowerCase() === normVal,
              );
              if (origVal) {
                resolved[k] = origVal;
                ...
                validVariants = validVariants.filter((v) => {
                  ...
                  return (
                    vKey &&
                    String(v.attributes[vKey]).trim().toLowerCase() === normVal
                  );
                });
              }
            }
  ...
  const checkIsValidOption = (attrName, attrValue) => {
    ...
    const attrIndex = attributeKeys.findIndex(
    ...
    return product.variants.some((v) => {
      ...
      return Object.entries(selectedAttributes).every(([k, selectedVal]) => {
        const kIndex = attributeKeys.findIndex(
          (key) => key.toLowerCase() === k.toLowerCase(),
        );
        if (kIndex >= attrIndex) return true;

        const vk = Object.keys(v.attributes || {}).find(
          (key) => key.toLowerCase() === k.toLowerCase(),
        );
        return (
          vk &&
          String(v.attributes[vk]).trim().toLowerCase() ===
            String(selectedVal).trim().toLowerCase()
        );
      });
    });
  ...
  const category = specs["Occasion"]
    ? `${specs["Occasion"]} Shirt`
    : "Casual Shirt";
